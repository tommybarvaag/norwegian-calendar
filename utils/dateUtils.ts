...
export const getFormattedDate = (date: Date, locale?: string): string => format(date, "dd-MM-yyyy", { locale: getLocale(locale) });

export const getFormattedShortDate = (date: Date, locale?: string): string => format(date, "EE d. MMM yyyy", { locale: getLocale(locale) });

export const getFormattedLongDate = (date: Date, locale?: string): string => format(date, "EEEE d. MMMM yyyy", { locale: getLocale(locale) });

export const getFormattedDay = (date: Date, locale?: string): string => format(date, "EEEE", { locale: getLocale(locale) });
...
export const getFormattedDateAndTime = (date: Date, locale?: string): string => `${getFormattedDate(date, locale)} ${getFormattedTime(date, locale)}`;

export const getFormattedIsoDateAndTime = (date: Date, locale?: string): string => format(date, "yyyy-MM-dd'T'HH:mm:ssXX", { locale: getLocale(locale) });

export const getWeek = (date: Date, locale?: string): number => getWeekDateFns(date, { locale: getLocale(locale), weekStartsOn: getWeekStarsOn(locale) });
